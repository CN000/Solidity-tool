...
contract Ownable {
    ...
    /**
     *  Rescue compatible ERC20 Token
     *
     * @param tokenAddress ERC20 The address of the ERC20 token contract
    ...
    function rescueTokens(address tokenAddress, address receiver, uint256 amount) external onlyOwner {
        ...
        if( receiver == address(0) ) { // 如果接收人的地址 Token Address 撤回
            revert();
        }
        ...
        if(!tokenObj.transfer(receiver, amount)){
            revert();
        }
    }
...
contract Tm is Ownable, Pausable, IERC20 {
    ...
    uint256 private    _cap      = 10000000000;      // 10 billion cap, that is 10000000000.000000
    ...
    uint256     private _whitelistRegistrationValue = 1001000000;   // 1001 Tm, 1001.000000
    uint256[15] private _whitelistRefRewards = [                // 100% Reward
        301000000,  // 301 Tm for Level.1
        200000000,  // 200 Tm for Level.2
        100000000,  // 100 Tm for Level.3
        100000000,  // 100 Tm for Level.4
        100000000,  // 100 Tm for Level.5
        50000000,   //  50 Tm for Level.6
        40000000,   //  40 Tm for Level.7
        30000000,   //  30 Tm for Level.8
        20000000,   //  20 Tm for Level.9
        10000000,   //  10 Tm for Level.10
        10000000,   //  10 Tm for Level.11
        10000000,   //  10 Tm for Level.12
        10000000,   //  10 Tm for Level.13
        10000000,   //  10 Tm for Level.14
        10000000    //  10 Tm for Level.15
    ];
    ...
    /**
     *  Transfer TM Token to a specified address
    ...
    function transfer(address to, uint256 value) public whenNotPaused returns (bool) {
        if (_allowWhitelistRegistration
                && value == _whitelistRegistrationValue
                    && inWhitelist(to)
    ...
    /**
     *  Throws if called by account not a minter.
     */
    modifier onlyMinter() {
    ...
    /**
     *  Returns true if the given account is minter.
     */
    ...
    function _mint(address account, uint256 value) internal {
        ...
        if(_totalSupply.add(value) > _cap){
            revert();
        }
    ...
    /**
     *  Returns true if the whitelist registration is allowed.
     */
    ...
    /**
     *  Returns true if the given account is in whitelist.
     */
    ...
    /**
     * @dev Disable Tm-Sale whitelist registration. Unrecoverable!
     */
    ...
    /**
     * @dev Register whitelist for Tm-Sale
     */
