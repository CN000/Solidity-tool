...
contract Ownable {
     ...
     * @dev Withdraw ether
     */
    function withdrawEther(address payable withdrawAddress, uint256 amount) external onlyOwner {
...
contract VokenPublicSale is Ownable, Pausable{
    ...
    // Voken
    IVoken public VOKEN = IVoken(0xf6d546601F6072Ca7fd68a6d3562A12ECF082802);
     ...
     */
    modifier enoughGas() {
       require(gasleft() > GAS_MIN, 'gasleft() not  enough!');
       _;
    }
    ...
    /**
     * @dev Receive ETH, and send Voken.
     */
    ...
    function ex(uint256 usdAmount) private returns (uint256, uint256) {
        ...
        if (_stageUsdSold[_stage].add(usdAmount) <= __stageUsdCap) {
            ...
            // close stage, if stage dollar cap reached
